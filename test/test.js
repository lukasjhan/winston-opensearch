...
const logMessage = JSON.parse(fs.readFileSync('./test/request_logentry_1.json', 'utf8'));

/*
 * Note: To run the tests, a running opensearch instance is required.
 */
...
function createLogger(buffering) {
  const logger = winston.createLogger({
    transports: [
      new winston.transports.Opensearch({
        flushInterval: 1,
        buffering,
...
describe('a buffering logger', () => {
  ...
  it('should log simple message to Opensearch', function (done) {
    this.timeout(8000);
    const logger = createLogger(true);
  ...
  it('should update buffer properly in case of an error from opensearch.', function (done) {
    this.timeout(80000);
    const logger = createLogger(true);
  ...
  describe('the logged message', () => {
    it('should be found in the index', function(done) {
      const opensearch = require('@open/opensearch');
...
describe('an Opensearch datastream', () => {
  it('should create a datastream called "logs-app-default" with default settings', function (done) {
    ...
    logger.on('finish', () => {
      sleep(5000).then(() => {
        new Client(clientOpts).indices
          .getDataStream({ name: 'logs-app-default' })
          .then(() => {
            done();
          })
          .catch((e) => {
  ...
  it('should create a datastream called "logs-myapp-mything" when using customization', function (done) {
    ...
    logger.on('finish', () => {
      sleep(5000).then(() => {
        new Client(clientOpts).indices
          .getDataStream({ name: 'logs-myapp-mything' })
          .then(() => {
            done();
          })
          .catch((e) => {
...
//     this.timeout(500000);
//     const transport = new (winston.transports.Opensearch)({
//       clientOpts: {
