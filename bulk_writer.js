...
const debug = require('debug')('winston:opensearch');
...
BulkWriter.prototype.append = function append(index, doc) {
  if (this.options.buffering === true) {
    ...
    this.write([{ [this.options.dataStream ? 'create' : 'index']: { _index: index, pipeline: this.pipeline } }, doc]);
  }
};
  ...
  return this.client
    ...
    .then((res) => {
      if (res && res.errors && res.items) {
        const err = new Error('Opensearch error');
        res.items.forEach((item, itemIndex) => {
          ...
          if (item[opKey] && item[opKey].error) {
            debug('Opensearch indexing error', item[opKey].error);
            thiz.options.internalLogger('Opensearch indexing error', item[opKey].error, bodyData);
    ...
    .catch((e) => {
      ...
      this.stop();
      this.checkEsConnection(thiz.retryLimit).catch((err) => thiz.transport.emit('error', err));
...
BulkWriter.prototype.ensureIndexTemplate = function ensureIndexTemplate(fulfill, reject) {
  const thiz = this;

  const indexPrefix =
    typeof thiz.options.indexPrefix === 'function' ? thiz.options.indexPrefix() : thiz.options.indexPrefix;
